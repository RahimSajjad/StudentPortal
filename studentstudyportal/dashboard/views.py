...
def notes(request):
    if request.method == "POST":
        ...
        messages.success(
            request, f"Notes added Successfully from {request.user.username}")
...
def homework(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            homeworks.save()
            messages.success(
                request, f"Homework added Successfully from {request.user.username}")
            return redirect("homework")
    else:
        form = HomeworkForm()

    homework = Homeworks.objects.filter(user=request.user)
    # Check homework done or not
    if len(homework) == 0:
        homework_done = True
    else:
        homework_done = False
    context = {"homeworks": homework,
               'homewor_done': homework_done, 'form': form}
    return render(request, 'dashboard/homework.html', context)

# Update check box make as complete
...
def todo(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            todo.save()
            messages.success(
                request, f"Todo added Successfully from {request.user.username}")
    ...
    if len(todo) == 0:
        ...
        todo_done = False
    context = {"todos": todo,
               'homewor_done': todo_done, 'form': form}
